...
const alreadyStyled = (active: InlineStyle, style: InlineStyle): boolean => {
  return styleKeys(style).every((key) => active[key]);
};

const canOpen = (text: string, index: number, marker: Marker): boolean => {
...
const canClose = (text: string, index: number, marker: Marker): boolean => {
...
const findSpanClose = (text: string, from: number, to: number): number => {
  let depth = 0;
  let index = from;
  while (index < to) {
    if (text[index] === "\\" && ESCAPABLE.has(text[index + 1])) {
...
export const parseInlineFormatting = (text: string): FormattedSegment[] => {
...
export const parseInlineSegments = (text: string): SourceSegment[] => {
