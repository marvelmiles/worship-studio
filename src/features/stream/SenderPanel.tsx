...
export function SenderPanel({ wantAudio, onBack }: { wantAudio: boolean; onBack: () => void }) {
  ...
  if (mode === "auto") {
    return <AutoBroadcastPanel wantAudio={wantAudio} onBack={onBack} onUseCode={() => setMode("manual")} />;
  }
  return (
    <ManualSenderPanel
      wantAudio={wantAudio}
...
function AutoBroadcastPanel({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia(CAMERA("environment", wantAudio));
      } catch {
        if (!cancelled) {
  ...
  const flipCamera = async () => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia(CAMERA(next, false));
      if (sender) {
    ...
    <div style={{ maxWidth: 460, margin: "0 auto" }}>
      <div style={{ background: colors.raise, border: `1px solid ${colors.border}`, borderRadius: 16, padding: 18 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: fonts.display, fontSize: 16, fontWeight: 600, color: colors.text }}>
...
function AutoStatusLine({ phase }: { phase: AutoPhase }) {
  const { colors, fonts } = useUITheme();
  const base = { display: "flex", alignItems: "center", gap: 8, marginTop: 12, fontFamily: fonts.ui, fontSize: 13 } as const;
  if (phase === "starting")
    return (
  ...
  if (phase === "waiting")
    return (
      <div style={{ ...base, color: colors.sub }}>
        <Spinner size={14} /> Waiting for the laptop to pick this camera. Open Stream → Display here on it.
      </div>
    );
  if (phase === "connecting")
    return (
  ...
  if (phase === "failed")
    return <div style={{ ...base, color: colors.danger }}>Connection dropped. Stop and start again, or use a code.</div>;
  return <div style={{ ...base, color: colors.sub }}>You're live — the laptop is projecting this camera.</div>;
}
...
function ManualSenderPanel({
  ...
  const startStreaming = async (offerSdp: string, useFacing: Facing) => {
    ...
    setPhase("streaming");
    try {
      const stream = await navigator.mediaDevices.getUserMedia(CAMERA(useFacing, wantAudio));
